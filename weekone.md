...
- Free with ads on [Tubi](https://tubitv.com/movies/289816/ghost-in-the-shell) and [Plex](https://watch.plex.tv/movie/ghost-in-the-shell)
- Also available via Hoopla (free with a public library card)
...
As an additional example, compare it with [**Future ELIZA 2501**](demos/WeekOne/ghost.html), built by handing the same prompt to a current model (Claude Fable 5, 2026) with a nudge toward the course's *Ghost in the Shell* framing. It is still nothing but pre-programmed pattern matching — no model runs behind the page — but notice the wider conversational range: it mirrors your own words back ELIZA-style, remembers topics you raised earlier and circles back to them, and varies its responses rather than repeating itself. The same prompt, one model generation later: does the ghost feel any closer, or is the shell just more elaborate?
...
Then, reflecting on the conversational AI systems you've just spent time with — original ELIZA, modern Claude, and the two generations of AI-generated Future ELIZA — alongside *Ghost in the Shell*: does ELIZA feel like it has anything resembling a ghost, or does it just feel like Weizenbaum's shell of a therapist, running its pattern-matching script? And what about the Puppet Master's claim to selfhood — is it different in kind from Claude's fluency, or just a difference of degree? Note any surprising moments across the conversations, and try to draw comparisons between them where you think about this week's lecture and readings, which situate generative AI in a history — a history we will be developing a better understanding of, especially throughout the first section of this course, even as we engage with very current and rapidly changing generative AI tools.
