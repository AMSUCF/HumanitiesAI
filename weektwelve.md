...
For both, connect your experience to Willison's lethal trifecta and to this week's epigraph. A local model is one way of removing a ghost from the network entirely — nothing to intercept, nothing to phone home to. An agent like Claude Code instead stays on the network, but (ideally) keeps its access to private data, untrusted content, and outbound communication from lining up all at once. Which of those two answers to the same worry did you end up trusting more this week, and what did you give up — in capability, convenience, or context — to get whichever kind of privacy or control you chose? Tyrangiel's fourth chapter, "Only Connect," frames AI's value around fostering human connection rather than replacing it — does a local model that runs off-network, answerable to no one and connected to nothing, sit comfortably inside that framing, or does "only connect" start to sound like the opposite of this week's whole point? Finally, what could AI agents — whether local or cloud-based — mean for humanities research, creative practice, or educational work?
